	...
	describe('with no records in the database', () => {
		describe('POST /books', () => {
			it('creates a new book in the database', async () => {
				...
				expect(newBookRecord.author).to.equal('Hideaki Anno');
	...
	describe('with records in the database', () => {
		...
		beforeEach(async () => {
			books = await Promise.all([
				...
				Book.create({
					title: '2.0 You Can (Not) Advance',
					author: 'Hideaki Anno',
					genre: 'mecha',
					ISBN: '9784905033035',
				}),
				Book.create({
					title: '3.0 You Can (Not) Redo',
					author: 'Hideaki Anno',
		...
		describe('PATCH /books/:id', () => {
			it('updates books genre by id', async () => {
				...
				expect(response.status).to.equal(200);
				expect(updatedBookRecord.genre).to.equal('sci-fi');
			});
